...
def _get_static_product_catalog() -> dict[str, Any]:
    ...
    This function and the module-level ``_STATIC_PRODUCT_CATALOG`` are kept
    here as the stable compat surface: tests patch
    ``ad_seller.interfaces.api.main._get_static_product_catalog`` and reset
    ``_STATIC_PRODUCT_CATALOG`` between tests, so endpoint code must resolve
    the catalog through this module at call time.
    """
